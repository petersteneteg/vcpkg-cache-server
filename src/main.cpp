...
auto logger(std::shared_ptr<spdlog::logger> log) {
    return [log](const httplib::Request& req, const httplib::Response& res) {
        log->debug("{:5} {:15} Path: {} Status: {} Vers: {}", req.method,
                   fp::mGet(req.headers, "REMOTE_ADDR").value_or("?.?.?.?"), req.path, res.status,
                   req.version);
    ...
    server.Get(R"(/cache/([0-9a-f]{64}))",
               [&](const httplib::Request& req, httplib::Response& res) {
                   ...
                   log->info(std::format(
                       ...
                       "Sha: {} ",
                       req.method, fp::mGet(req.headers, "REMOTE_ADDR").value_or("?"),
                       info->package, info->version, info->arch, ByteSize{info->size}, info->time,
